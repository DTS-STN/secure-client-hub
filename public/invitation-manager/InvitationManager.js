...
function imSetup() {
  ...
  function mainPart1() {
      ...
      if (!surveyDB) {
        ...
        if (localStorage.getItem('lastDateIMShown')) {
          var cookieName = 'lastDateIMShown',
            maxNbDaysIMPersist = new Date(
              localStorage.getItem('lastDateIMShown')
            )
          maxNbDaysIMPersist.setDate(
            maxNbDaysIMPersist.getDate() + surveyDB.settings['duration-delay']
          )
          setCookie(
            cookieName,
            localStorage.getItem('lastDateIMShown'),
            maxNbDaysIMPersist,
            removeSubdomain(window.location.href.toLowerCase())
          )
        } else {
          mainPart2()
        }
    ...
    // Case when the user has already seen an invitation
      ...
      localStorage.setItem(
        'lastDateIMShown',
        new Date(getCookie('lastDateIMShown'))
      )
    }
  }
  ...
  function mainPart2() {
    ...
    if (!surveyDB || !surveyDB.settings) return
    ...
    if (getCookie('lastDateIMShown')) {
      ...
      if (isStorageExpired(lastDateIMShown)) {
        document.cookie =
          'lastDateIMShown=;expires=Thu, 01 Jan 1970 00:00:00 GMT; Secure;domain=' +
          removeSubdomain(window.location.href.toLowerCase()) +
          ';path=/'
    ...
    consoleLog(
      'Scope hat = ' +
        surveyScope +
        ', ' +
        surveyDB.surveys.length +
        ' potential surveys, removing surveys outside of date range'
    )
    ...
    consoleLog(
      list.length + ' potential surveys, removing surveys based on targeting'
    )
    ...
    if (b) {
      ...
      if (
        !getCookie('lastDateIMShown') &&
        !/[?&]im_nocookiecheck=1/i.test(document.location.search)
      ) {
      ...
      consoleLog(
        'No survey selected, ' +
          surveyDB.surveys.length +
          ' surveys remain in session storage'
      )
  ...
  function firstTimeSetup() {
    ...
    if (overrideScope) {
      ...
      surveyScope = getWeightedRandom({
        'Page': surveyDB.settings['page-probability'],
        'Entire site': surveyDB.settings['site-probability'],
      })
    ...
    consoleLog(
      list.length +
        " potential surveys, removing surveys that don't match scope hat"
    )
    ...
    tmpSurveyDB.settings['page-probability'] =
      surveyDB.settings['page-probability']
    tmpSurveyDB.settings['site-probability'] =
      surveyDB.settings['site-probability']
    ...
    for (var i = 0; i < surveyDB.surveys.length; i++) {
      ...
      switch (survey_i.type) {
        ...
        case 'Page':
          ...
          for (var c = 0; c < sublist.length; c++) {
            if (
              wLocation.indexOf(sublist[c].url.toLowerCase()) !== -1 &&
              i >= 0
            ) {
              surveySubList[survey_i.id] =
                survey_i.probability * surveyDB.settings['page-probability']
              count++
  ...
  function invite(survey) {
    ...
    if (Adobe.toLowerCase() === 'yes') {
      popupInput =
        "<input type='hidden' name='popupName' value='" + survey['name'] + "'>"
    } else {
      popupInput = ''
    }
    ...
    var html =
        ...
        "<div class='gc-im-modal-title-fix1'>" +
        survey['title-' + wb_im.lang] +
        '</div>' +
        "<div class='gc-im-modal-title-fix2'>" +
        ...
        "<li class='mrgn-sm marginBottom-yes'><a id='survey-yes' class='gc-im-btn gc-im-btn-primary' href='" +
        survey['link-' + wb_im.lang] +
        "' target='_blank' referrer-policy='unsafe-url'>" +
        survey['yes-' + wb_im.lang] +
        '</a></li> ' +
        "<li class='mrgn-sm marginBottom-no'><button id='survey-no' class='gc-im-btn gc-im-btn-secondary survey-close'>" +
        survey['no-' + wb_im.lang] +
        '</button></li>' +
        '</ul>' +
        popupInput +
        '</div>' +
    ...
    $html.bind('click vclick mouseup keydown', function (e) {
      ...
      if (
        ((e.type === 'click' || e.type === 'vclick') &&
          e.which === 1 &&
          $(e.target).closest('a, button', this).length) || // Clicked/Tapped a link/button.
        (e.type === 'mouseup' &&
          e.which === 2 &&
          $(e.target).closest('a', this).length) || // Middle-clicked a link.
      ...
      if (e.type === 'keydown' && e.which === 9 && e.which === 16) {
        // Pressed the tab key and shift key
    ...
    if (wb_im.lang === 'fr') {
      $('#wb-tphp').prepend(
        "<li id='first-focus' class='wb-slc'>" +
  ...
  function getCookie(name) {
    if (
      ...
      return document.cookie
        .split('; ')
        .find((row) => row.startsWith(name.trim()))
        .split('=')[1]
    }
    return false
  }
